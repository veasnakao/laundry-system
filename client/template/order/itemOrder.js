...
Template.itemOrder.events({
            ...
            Meteor.call('updateOrder', orderId, updateObj, (error, result)=> {
                if (error) {
                    ...
                    if (order) {
                        ...
                        let staffId = order.staffId;
                        Router.go(`/itemOrder/orderId/${orderId}/staffId/${staffId}/customerId/${customerId}/printInvoice`);
                    }
                }
            });
        }
    ...
    'change [name="deliveryDate"]'(){
        let deliveryDate = $('.deliveryDate').val();
        let staffId = $('.js-staff').val();
        let params = Router.current().params;
        let orderId = params.orderId;
        if (deliveryDate.length === 0 || staffId.length === 0) {
    ...
    'change [name="receiveDate"]'(){
        let receiveDate = $('.receiveDate').val();
        let staffId = $('.js-staff').val();
        let params = Router.current().params;
        let orderId = params.orderId;
        if (receiveDate.length === 0 || staffId.length === 0) {
            $('.search-item').hide(300);
        } else {
            $('.search-item').show(300);
        }
        let updateObj = {};
        if (receiveDate && orderId) {
            updateObj.receiveDate = receiveDate;
            Meteor.call('updateOrder', orderId, updateObj, (error, result)=> {
                if (error) {
                    swal({
                        title: "Error",
        ...
        if (customerId && orderId) {
            ...
            Meteor.call('updateOrder', orderId, updateObj, (error, result)=> {
                if (error) {
                    ...
                    Meteor.call('serviceReport', orderId, (error, result)=> {
                        if (error) {
                            swal({
                                title: "Error",
                                text: error,
